...
export default function BusinessFarmingCommitmentPage() {
    ...
    const filteredCommitments = farmingCommitments.filter(commitment =>
        ...
        (!search || commitment.commitmentName.toLowerCase().includes(search.toLowerCase()))
    );

    const totalPages = Math.ceil(filteredCommitments.length / pageSize);
    const pagedCommitments = filteredCommitments.slice((currentPage - 1) * pageSize, currentPage * pageSize);

    const statusCounts = filteredCommitments.reduce<Record<FarmingCommitmentStatusValue, number>>((acc, commitment) => {
        ...
        <div className="flex bg-amber-200-50 p-6 gap-6">
            ...
            <aside className="w-64 space-y-4">
                ...
                <div className="bg-white rounded-xl shadow-sm p-4 space-y-4">
                    <h2 className="text-sm font-medium text-gray-700">Tìm kiếm cam kết</h2>
            ...
            <main className="flex-1 space-y-6">
                ...
                <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">
                        Hiển thị {(currentPage - 1) * pageSize + 1}–{Math.min(currentPage * pageSize, filteredCommitments.length)} trong {filteredCommitments.length} kết quả
                    </span>
